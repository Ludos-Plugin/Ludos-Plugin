...
import fr.ludos.core.group.gui.item.InvitePlayerItem;
 ...
 */
public class GroupInviteGui implements WindowObject {
	private final GroupManager manager;

	public GroupInviteGui(GroupManager manager) {
	...
	public Window window(Player player, GuiContext context) {
		...
		List<Item> items = manager.getLudos().getServer().getOnlinePlayers().stream()
			...
			.filter(Predicate.not(group::isPlayer))
			.map(target -> new InvitePlayerItem(manager, target))

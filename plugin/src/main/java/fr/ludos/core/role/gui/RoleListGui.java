 ...
 */
public class RoleListGui implements WindowProvider {
	private final RoleManager manager;

	public RoleListGui(RoleManager manager) {
	...
	public Window window(Player player, GuiContext context) {
		...
		List<Item> items = manager.getBuilders().stream()
			.map(g -> (Item) RoleGui.item(g, manager, childrenContext).addActionHandler(settings::disableModalReturn))
			.toList();

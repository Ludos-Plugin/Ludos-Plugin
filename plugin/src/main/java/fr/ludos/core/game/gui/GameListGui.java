 ...
 */
public class GameListGui implements WindowProvider {
	private final GameManager manager;

	public GameListGui(GameManager manager) {
	...
	public Window window(Player player, GuiContext context) {
		...
		List<Item> items = manager.getBuilders().stream()
			.map(g -> (Item) GameGui.item(g, manager, childrenContext).addActionHandler(settings::disableModalReturn))
			.toList();
